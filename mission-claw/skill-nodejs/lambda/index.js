...
const MoveIntentHandler = {
    ...
    handle: function (handlerInput) {
        ...
        // Degrees is optional, use default if not available
        const degrees = Alexa.getSlotValue(request, 'Degrees') || "90";
        const angle = degrees * 10;
        ...
        //     ?  "Applying brake"
        //     : `${direction} ${duration} seconds or ${degrees} degrees at ${speed} percent speed`;
        var speechOutput = '';
        switch(direction){
            case 'break':
            speechOutput = 'Applying break';
            case 'turn around':
                    speechOutput = 'Turning around';
            break;
            case 'right':
            case 'go right':
            case 'left':
            case 'go left':
                speechOutput = `${direction} turn at ${degrees} degrees`;
                break;
            default:
            speechOutput = `${direction} ${duration} seconds at ${speed} percent speed`;

        }
        const reprompt = PAUSE + 'awaiting command';
        return handlerInput.responseBuilder
            .speak(speechOutput)
            .reprompt(reprompt)
...
const SetCommandIntentHandler = {
    ...
    handle: function (handlerInput) {
        ...
        else if (command == 'wine' || command == 'water'|| command == 'beer' ) 
            {
                command = "fetching " + command; 
        }
...
const thankUIntentHandler = {
    canHandle(handlerInput) {
        return Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest'
            && Alexa.getIntentName(handlerInput.requestEnvelope) === 'thankU';
    },
    handle: function (handlerInput) {

        return handlerInput.responseBuilder
            .speak(`You're welcome Caroline!`)
            .getResponse();
    }
};
