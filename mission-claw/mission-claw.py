...
class MindstormsGadget(AlexaGadget):
    ...
    def on_custom_mindstorms_gadget_control(self, directive):
        ...
        try:
            ...
            if control_type == "move":
                ...
                # Expected params: [direction, duration, degrees, speed]
                self._move(payload["direction"], int(payload["duration"]), int(payload["degrees"]), int(payload["speed"]))
    ...
    def _move(self, direction, duration: int, degrees: int, speed: int, is_blocking=False):
        """
        ...
        """
        print("Move command: ({}, {}, {}, {}, {})".format(direction, speed, duration, degrees, is_blocking), file=sys.stderr)
    ...
    def _activate(self, command, speed=50):
        ...
        if command in Command.WATER.value:
            self.claw_open(True)                    
            self.drive.on_for_seconds(SpeedPercent(60), SpeedPercent(60), 2)
            self.claw_close(True)
            self._turn("left", 1750, speed)
            self.drive.on_for_seconds(SpeedPercent(60), SpeedPercent(60), 2)
            self.claw_open(True)
